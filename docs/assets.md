...
Theme — meadow/forest/sewer/crypt/town/glacier) are original pixel-art PNG tiles. The Glacier
Frostspire set is now original deterministic pixel art generated by the committed
`scripts/art/backdrops.mjs` registry, wired through `scripts/art/generate.mjs` and `npm run art`.
Its earlier #254 version was procedurally hand-assembled with a small throwaway script; #293 keeps
that provenance while replacing those bytes with the committed, reproducible generator. The five
other themes remain their historical hand-assembled files. Glacier draws only from its
`glacier` zone sub-palette (`scripts/art/palettes.mjs`) and master neutrals. Every repeating shape (cloud,
...
art:overlays` rebuilds all five once their raw inbox generations are present. `npm run art` rebuilds
the committed deterministic art registries, including Glacier's backdrop layers.
