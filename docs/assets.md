...
live in `scripts/art/icons.mjs` (paint functions on the shared `scripts/art/icon-canvas.mjs` 34×34
canvas) and are regenerated byte-stably by `npm run art`, which now also writes
...
`docs/art-style.md` / `scripts/art/palettes.mjs` — no new hex values were introduced. Each icon is a
...
`tab-bank` a coin stack, `tab-vendor` a coin purse, `tab-loot` a scroll. The four production views
