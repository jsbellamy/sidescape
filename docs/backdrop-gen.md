...
`scripts/art/backdrops.mjs` holds one entry per shipped backdrop slice. Uniqueness is **`(theme, variant)`**, not theme alone — a Theme may register both a **base entry** (the default sky/mid/near stack) and optional **variant entries** beside it.

**Base entry** — exactly three layers: `sky` (opaque), `mid` (binary alpha), `near` (binary alpha). No `variant` field.

**Fishing variant entry** — declares `variant: "fishing"` and its `layers` object contains exactly one layer named `near-fishing` with `alpha: "binary"`. A variant entry declares its own `gamut`; it does not inherit from the base entry. Stage 2 ships it as `src/assets/backdrops/<theme>-near-fishing.png`.

A base entry with a fourth layer, or a fishing variant with any layer set other than `near-fishing`, is rejected. Duplicate `(theme, variant)` pairs are rejected.

## Ingest

Register the entry in `backdrops.mjs` first, then ingest one untouched raw at a time:

```sh
npm run art:ingest-backdrop -- --theme <theme> --layer <sky|mid|near|near-fishing>
```

For `sky`, `mid`, or `near`, ingest resolves the **base** entry for that Theme. For `near-fishing`, ingest resolves the **fishing variant** entry (`variant: "fishing"`) for that Theme.
...
For a fishing variant, `<layer>` is `near-fishing` (for example `glacier-near-fishing.png`).

Every `kind: "source"` entry declares one HSL gamut beside `theme`, `kind`, and `layers` (and `variant` when present). The same gamut governs every layer on that entry — never hardcode a Theme name or duplicate gamut per layer. Prompts guide palette intent but do not enforce decoded-RGB bounds; conformance does.
...
### Keyed transparent mid, near, or near-fishing layer
...
For `near-fishing`, use the same keyed prompt shape but describe shoreline/water-edge terrain drawn into the near layer (the fishing activity swaps this variant tile in place of the base `near` layer at runtime).
