...
import { spellLadder, spellLadderRunes, spellLadderVendor } from "./spell-ladder";
...
export const content: Content = composeContent(
  [lumbryMeadows, darkrootForest, oldSewers, boneCrypt, frostspire],
  {
    items: [
      ...
      // meaningful, not bankrupting. Magic mirrors this at 1 rune/cast: a Strike rune's 3g price
      // (below, picked to match bronze-arrow's own ratio rather than the issue's illustrative 4g) x
      // ~2,000 casts/hr is also ~6,000g/hr, the same ~33% ratio — so switching between a bow and a
      // staff (see the Rune Slot's own "no reload" test) never trades into a cheaper or pricier
      // resource sink, only a different one. steel-/mithril-arrow scale the SAME ratio at their own
      // tier's higher assumed gold income; Bolt runes (6g, #364) and Blast runes (20g, #364) extend
      // that band to the higher incomes where Magic 17–35 and 41–59 unlock — ~12,000g/hr and
      // ~40,000g/hr at ~2,000 casts/hr respectively, still landing in the ~30–40% gold-sink target
      ...
      // Spell ladder (#364): eight Bolt/Blast runes appended after rune-platelegs (append-only —
      // existing item ids above keep their exact array index). Strike runes stay in the block above;
      // spellLadderRunes() covers Bolt and Blast only.
      ...spellLadderRunes(),
      // Wave A arrow ladder (#365): iron-arrow appended after #364's runes (append-only — every
      // pre-existing item id above keeps its exact array index). Fills the iron tier gap so
      // iron-shortbow (Ranged 5) has matching ammo at the same wear level.
      {
        kind: "ammo",
        id: "iron-arrow",
        name: "Iron Arrow",
      ...
      // Wave content (#387): raw chicken / silk / cave eel / icefin Materials plus cooked Food tiers
      // — appended after iron-arrow (append-only). Silk is sell-only interim sink until magic robes.
      { kind: "material", id: "raw-chicken", name: "Raw Chicken", icon: "raw-chicken", value: 1 },
      { kind: "material", id: "silk", name: "Silk", icon: "silk", value: 4 },
      {
        kind: "material",
        id: "raw-cave-eel",
        name: "Raw Cave Eel",
      ...
      { kind: "material", id: "raw-icefin", name: "Raw Icefin", icon: "raw-icefin", value: 8 },
      {
        kind: "food",
        id: "cooked-chicken",
        name: "Cooked Chicken",
    ...
    recipes: [
      ...
      // way off Sewer Outflow's catch rate (catchChance 0.45, catchTicks 5 -> ~0.09 raw-pike/tick),
      // not off a faster Cooking action.
      {
        id: "cook-beef",
        name: "Cook Beef",
        skill: "cooking",
    ...
    // Spell ladder (#364): twelve elemental spells (Strike/Bolt/Blast × four elements), generated
    // by spell-ladder.ts — Strike row reproduces shipped numbers exactly.
    spells: spellLadder(),
    vendor: [
