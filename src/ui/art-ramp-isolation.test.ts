 ...
 * per-asset scoping (`icons.mjs`'s SOURCE_RAMPS, `sprites.mjs`'s per-sprite `ramps`) it passes
 * because no asset declares the decoy.
 */
  ...
  it("adding a new material ramp leaves every committed icon byte-identical", async () => {
    ...
    for (const [name, bytes] of icons) {
      expect(
        ...
        `icon "${name}" changed when an unrelated material ramp was added — quantization is not scoped to the ramps this icon's source declares (scripts/art/icons.mjs SOURCE_RAMPS)`,
      ).toBe(0);
    }
  });
