...
const EXPECTED_SPRITES = [
  ...
  { name: "cow", size: 48, alpha: "binary" },
  { name: "goblin", size: 32, alpha: "binary" },
  { name: "goblin-brute", size: 48, alpha: "binary" },
  { name: "goblin-chief", size: 48, alpha: "binary" },
  ...
  { name: "ice-wraith", size: 32, alpha: "binary" },
  { name: "frost-giant", size: 32, alpha: "binary" },
  { name: "frost-warden", size: 48, alpha: "binary" },
] as const;
...
describe("buildSpriteContactSheets", () => {
  it("derives its cell edge from mixed native canvases without cropping 64px edge pixels", () => {
    ...
    writeFileSync(
      ...
      encodePng(64, 64, (x: number, y: number) => {
        const edge =
          ...
          (x === 0 && y === 63) ||
          (x === 63 && y === 63);
        return edge ? [0, 255, 0, 255] : [0, 0, 0, 0];
      }),
    );
    ...
    const repeated = buildSpriteContactSheets(root, [
      { name: "small", size: 32 },
      { name: "medium", size: 48 },
      { name: "large", size: 64 },
    ]);
    ...
    expect([oneX.width, oneX.height]).toEqual([132, 132]);
    const pixel = (png: PNG, x: number, y: number) => {
      const at = (y * png.width + x) * 4;
      return [...png.data.subarray(at, at + 4)];
    };
    expect(pixel(oneX, 16, 32)).toEqual([255, 0, 0, 255]);
    expect(pixel(oneX, 76, 16)).toEqual([0, 0, 255, 255]);
    expect(pixel(oneX, 0, 68)).toEqual([0, 255, 0, 255]);
    expect(pixel(oneX, 63, 68)).toEqual([0, 255, 0, 255]);
    expect(pixel(oneX, 0, 131)).toEqual([0, 255, 0, 255]);
    expect(pixel(oneX, 63, 131)).toEqual([0, 255, 0, 255]);
    expect(pixel(oneX, 0, 0)).toEqual([0x26, 0x20, 0x19, 255]);

    const fourX = PNG.sync.read(built.fourX);
    expect([fourX.width, fourX.height]).toEqual([528, 528]);
    expect(pixel(fourX, 3, 68 * 4 + 3)).toEqual([0, 255, 0, 255]);
    expect(pixel(fourX, 63 * 4 + 3, 68 * 4 + 3)).toEqual([0, 255, 0, 255]);
    expect(pixel(fourX, 3, 131 * 4 + 3)).toEqual([0, 255, 0, 255]);
    expect(pixel(fourX, 63 * 4 + 3, 131 * 4 + 3)).toEqual([0, 255, 0, 255]);
    expect(Buffer.compare(built.oneX, repeated.oneX)).toBe(0);
    expect(Buffer.compare(built.fourX, repeated.fourX)).toBe(0);
  });
